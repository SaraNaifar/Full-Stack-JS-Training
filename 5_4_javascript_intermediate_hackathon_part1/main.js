...
/*  Find the Longest Word in a String 
	...
	*/
	function findLongestWord(str){
		...
		let maxLengthString = splitedString[0].length ;
		for(let i = 0; i < splitedString.length; i++){
			if(splitedString[i].length > maxLengthString){
				maxLengthString = splitedString[i].length;
			}
		}
	...
	*/
	function titleCase(str) {
		...
		for(let i=0 ; i<capitalizedString.length ; i++){
			if ((capitalizedString[i-1]==" ")||(i==0)){
				newstr=newstr+capitalizedString[i].toUpperCase();
			}else{
				newstr=newstr+capitalizedString[i];
			}
...
/*  Confirm the Ending
	...
	*/
	function confirmEnding(str, target) {
		...
		let sliceEndingStr = str.slice(str.length-lengthTarget , str.length);
		return (sliceEndingStr===target);
	}
	...
	*/
	function truncateString(str, num) {
		...
		if (num<=3){
			...
			let newStr = slicedStr+"...";

			return newStr ;

		}else if (str.length>num){

			let slicedStr = str.slice(0,num-3);
			let newStr = slicedStr+"...";
...
/*  Mutations
	...
	*/
	...
	function mutation(arr) {
		...
		let firstElt=arr[0].toLowerCase();
		let secondElt=arr[1].toLowerCase();
		
		for(let i =0; i<secondElt.length ; i++){

			if(!includes(firstElt , secondElt[i])){
	...
	function mutationV(arr) {

		let firstElt=arr[0].toLowerCase();
		let secondElt=arr[1].toLowerCase();
		
		secondElt.split ('').every(function isInCh(c){
			return includes(secondElt , colno);
		});
