	...
	*/
	function tag_count(stringList) {
		...
		for(var i=0 ; i<stringList.length ; i++){
			//for(var j=0 ; j<stringList[i].length ;j++){
				if (stringList[i][0]=="<" && stringList[i][stringList[i].length-1]==">" ){
					count+=1;
...
/*	Create an HTML List
    ...
    */
    function html_list(stringList) {
    	...
    	for (var i=0 ; i<stringList.length; i++){
    		//for (var j=0 ; j<stringList[i].length ; j++){
    			newstr = newstr+ "<li>" +stringList[i]+"</li>";

    		}
    		var result ="<ul>"+newstr+"</ul>";
    		return result;
	...
	*/
	function news_ticker(headlines) {
		...
		for (var i=0 ; i<headlines.length ; i++){
			...
			var len=len+ticker.length;
		}
		//var newticker= ticker.slice(0,142);
		return ticker;
	...
	function front_x(words) {
		...
		for (var i=0 ; i<=words.length-1; i++){
			if (words[i][0]=="x"){
				...
				list_letter.push(words[i]);
			}
		}
		var list_sorted_x=list_x.sort();
		var list_sorted_letter=list_letter.sort();
		var final_list = list_sorted_x.concat(list_sorted_letter);
		return final_list;
	}
	print("front_x");
...
/*
	...
	*/
	...
	function linear_merge(list1, list2) {
		var sorted_list= list1.concat(list2);
		for(var i=0 ;i<sorted_list.length ; i++){
			if (sorted_list[i][0]>=sorted_list[i+1][0]){

				var aux = sorted_list[i];
